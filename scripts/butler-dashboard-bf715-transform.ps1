...
if (Test-Path -LiteralPath $stagedCore -PathType Leaf) {
    ...
    # BF-945: route ready Matchup lineup decisions into the richer My Team
    # Lineup Advisor after BF-944 finalizes compare return context.
    $bf945Transform = Join-Path $PSScriptRoot 'butler-app-bf945-matchup-lineup-review-bridge-transform.ps1'
    if (-not (Test-Path -LiteralPath $bf945Transform -PathType Leaf)) {
        throw "BF-945 BLOCKED: Matchup Lineup Review Bridge transform not found at $bf945Transform"
    }
