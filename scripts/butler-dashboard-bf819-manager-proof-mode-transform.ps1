    ...
    for ($managerIndex = 0; $managerIndex -lt $orderedPrioritySignals.Count; $managerIndex++) {
        ...
        switch ($kind) {
            ...
            "Waiver" {
                switch ([string]$state) {
                    "CURRENT_AND_ACTIONABLE" {
                        ...
                        $status = "MOVE READY"; $statusClass = "good"; $actionHref = "/waivers"; $actionLabel = "Review Waiver Move"
                    }
                    "CURRENT_REFRESH_RECOMMENDED" {
                        ...
                        $status = "REFRESH"; $statusClass = "warn"; $actionHref = "/waivers"; $actionLabel = "Open Waiver Board"
                    }
                    "TRANSACTION_PENDING_DO_NOT_DUPLICATE" {
                        ...
                        $status = "WAIT"; $statusClass = "warn"; $actionHref = "/history"; $actionLabel = "View History"
                    }
                    "TRANSACTION_ALREADY_COMPLETE" {
                        ...
                        $status = "COMPLETE"; $statusClass = "done"; $actionHref = "/history"; $actionLabel = "View History"
                    }
                    "STALE_DO_NOT_ACT" {
                        $title = "Do not act on the saved waiver move"
                        $copy = "The saved move no longer passes Butler's current safety checks."
                        $status = "DO NOT ACT"; $statusClass = "danger"; $actionHref = "/waivers"; $actionLabel = "Open Waiver Board"
                    }
                    "NO_TRANSACTION_TO_ACT_ON" {
                        $title = "No waiver move proven"
                        $copy = "Current review found no add/drop strong enough to recommend."
                        $status = "NO MOVE"; $statusClass = "done"; $actionHref = "/waivers"; $actionLabel = "Waiver Board"
                    }
                    default {
                        $title = "No waiver decision available"
                        $copy = "Butler does not have a current waiver move to act on."
                        $status = "UNAVAILABLE"; $statusClass = "warn"; $actionHref = "/waivers"; $actionLabel = "Waiver Board"
                    }
                }
            ...
            "Trade" {
                ...
                $status = "ON DEMAND"; $statusClass = "done"; $actionHref = "/trade"; $actionLabel = "Trade Lab"
                $extraHtml = ""
            }
        }
...
<article class="$cardClass">
  ...
  <div class="manager-decision-main"><div class="manager-kind">$(ConvertTo-HtmlText $kind)</div><h3>$(ConvertTo-HtmlText $title)</h3><p>$(ConvertTo-HtmlText $copy)</p>$chipRow$extraHtml<div class="manager-card-actions"><a class="command-button" href="$(ConvertTo-HtmlText $actionHref)">$(ConvertTo-HtmlText $actionLabel)</a>$secondaryActions</div></div>
        ...
        switch ([string]$priorityOne.Kind) {
            "Lineup" {
                $proofChangeCopy = switch ([string]$lineupSignalStatus) {
                    ...
                    "AUTOFILL READY" { "The latest lineup review found projected starter changes worth reviewing." }
