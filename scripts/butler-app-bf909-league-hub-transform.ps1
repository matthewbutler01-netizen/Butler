    ...
    if ($View.MovementAvailable) {
        if ($View.Movers.Count -eq 0) {
            ...
            foreach ($mover in $View.Movers) {
                $moverMatch = [regex]::Match([string]$mover, '^(?<delta>[+-][0-9.]+)\s+(?<position>\S+)\s+(?<player>.*?)\s+(?<nfl>\S+)\s+fantasy-team=(?<team>.*?)\s+(?<previous>-?[0-9.]+)\s+->\s+(?<latest>-?[0-9.]+)(?:\s+\[[^\]]+\])?\s*
        }
    }
    else {
...
Write-Host 'BF-909 manager-first League Hub applied.'
                ...
                if ($moverMatch.Success) {
                    $delta = $moverMatch.Groups['delta'].Value
                    $deltaClass = if ($delta.StartsWith('+')) { 'positive' } else { 'negative' }
                    $movementHtml += @"
<div class="league-mover-row">
  <div class="league-mover-main"><span class="league-mover-delta $deltaClass">$(ConvertTo-HtmlText $delta)</span><div><strong>$(ConvertTo-HtmlText $moverMatch.Groups['player'].Value)</strong><span>$(ConvertTo-HtmlText $moverMatch.Groups['position'].Value) &middot; $(ConvertTo-HtmlText $moverMatch.Groups['nfl'].Value)</span></div></div>
  <div class="league-mover-context"><span>$(ConvertTo-HtmlText $moverMatch.Groups['team'].Value)</span><strong>$(ConvertTo-HtmlText $moverMatch.Groups['previous'].Value) &rarr; $(ConvertTo-HtmlText $moverMatch.Groups['latest'].Value)</strong></div>
</div>
"@
                }
                else {
                    $cleanMover = [regex]::Replace([string]$mover, '\s+\[[^\]]+\]\s*
        }
    }
    else {
        $movementHtml = '<div class="empty">Value movement is unavailable until comparable provider snapshots exist. Butler does not manufacture a trend.</div>'
...
Write-Host 'BF-909 manager-first League Hub applied.'
, '')
                    $movementHtml += "<div class=`"league-mover-row`">$(ConvertTo-HtmlText $cleanMover)</div>"
                }
            }
        }
    }
