    ...
    if ($View.MovementAvailable) {
        if ($View.Movers.Count -eq 0) {
            ...
            foreach ($mover in $View.Movers) {
                $moverMatch = [regex]::Match(
                    [string]$mover,
                    '^(?<delta>[+-][0-9.]+)\s+(?<position>\S+)\s+(?<player>.*?)\s+(?<nfl>\S+)\s+fantasy-team=(?<team>.*?)\s+(?<previous>-?[0-9.]+)\s+->\s+(?<latest>-?[0-9.]+)(?:\s+\[[^\]]+\])?\s*$'
                )
...
"@
                    ...
                    $movementHtml += @"
<div class="league-mover-row">$(ConvertTo-HtmlText $cleanMover)</div>
"@
                }
            }
        }
