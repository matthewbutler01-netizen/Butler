...
if (-not [string]::IsNullOrWhiteSpace($CorePath)) {
    ...
    $coreKnownOld = '$candidate = $path -match ''^/waivers/candidate/[0-9]+
...
'`r`n            $knownDashboardPath = $path -eq "/" -or $path -eq "/index.html" -or $path -eq "/waivers" -or $candidate'
    $coreKnownNew = '$candidate = $path -match ''^/waivers/candidate/[0-9]+
$cardActionOld = '<div class="actions" style="margin-top:12px"><a class="button" href="/waivers/candidate/$(ConvertTo-HtmlText $candidate.SleeperId)">View governed details</a></div>'
$cardActionNew = '<div class="actions" style="margin-top:12px"><a class="button" href="/waivers/candidate/$(ConvertTo-HtmlText $candidate.SleeperId)">View governed details</a><a class="button" href="/waivers/compare?left=$(ConvertTo-HtmlText $candidate.SleeperId)">Compare candidate</a></div>'
$text = Replace-ExactlyOnce -Text $text -Old $cardActionOld -New $cardActionNew -Contract 'Waiver Board compare entry action'
...
Write-Host 'BF-939 Waiver Candidate Compare applied.'
'`r`n            $waiverCompare = $path -eq "/waivers/compare"`r`n            $knownDashboardPath = $path -eq "/" -or $path -eq "/index.html" -or $path -eq "/waivers" -or $waiverCompare -or $candidate'
    $core = Replace-ExactlyOnce -Text $core -Old $coreKnownOld -New $coreKnownNew -Contract 'app-shell Waiver Candidate Compare allowlist'

    $proxyOld = '$dashboardRequestTarget = if ($path -eq "/waivers") { $parts[1] } else { $path }'
    $proxyNew = '$dashboardRequestTarget = if ($path -eq "/waivers" -or $path -eq "/waivers/compare") { $parts[1] } else { $path }'
    $core = Replace-ExactlyOnce -Text $core -Old $proxyOld -New $proxyNew -Contract 'app-shell compare query preservation'
