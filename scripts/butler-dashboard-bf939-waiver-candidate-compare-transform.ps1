...
if (-not [string]::IsNullOrWhiteSpace($CorePath)) {
    if (-not (Test-Path -LiteralPath $CorePath -PathType Leaf)) {
        throw "BF-939 BLOCKED: staged Butler core not found at $CorePath"
    }
...
'@
    $core = Replace-ExactlyOnce -Text $core -Old $candidateLineOld.TrimEnd() -New $candidateLineNew.TrimEnd() -Contract 'app-shell compare route marker'
...
'@
    $core = Replace-ExactlyOnce -Text $core -Old $knownOld.TrimEnd() -New $knownNew.TrimEnd() -Contract 'app-shell compare allowlist'
...
'@
    $core = Replace-ExactlyOnce -Text $core -Old $proxyOld.TrimEnd() -New $proxyNew.TrimEnd() -Contract 'app-shell compare query preservation'

    foreach ($required in @(
        '$waiverCompare = $path -eq "/waivers/compare"',
        '$path -eq "/waivers/compare") { $parts[1] }',
        'Invoke-GovernedDashboardGet -InnerPort $innerPort -Path $dashboardRequestTarget'
    )) {
        if ($core.IndexOf($required, [System.StringComparison]::Ordinal) -lt 0) {
            throw "BF-939 BLOCKED: required app-shell compare marker is missing: $required"
        }
    }
    ...
    if (@($parseErrors).Count -gt 0) {
        ...
        throw "BF-939 BLOCKED: generated staged core failed PowerShell parse: $parseSummary"
    }
}
...
'@
$text = Replace-ExactlyOnce -Text $text -Old $cardActionOld.TrimEnd() -New $cardActionNew.TrimEnd() -Contract 'Waiver Board compare entry action'
...
'@
$text = Replace-ExactlyOnce -Text $text -Old $knownOld.TrimEnd() -New $knownNew.TrimEnd() -Contract 'dashboard compare allowlist'
...
$routeNew = @'
                ...
                elseif ($candidateMatch.Success) {
                    $candidateId = $candidateMatch.Groups['id'].Value
'@
$text = Replace-ExactlyOnce -Text $text -Old $routeOld.TrimEnd() -New $routeNew.TrimEnd() -Contract 'dashboard compare route'
...
if ($installed -match 'Invoke-RestMethod|Invoke-WebRequest|https://api\.sleeper\.app|Method = "POST"|submitTransaction|setFaab') {
    throw 'BF-939 BLOCKED: Waiver Candidate Compare introduced provider, write, or FAAB behavior.'
}
