    ...
    if ($question -ge 0 -and $question -lt ($RequestTarget.Length - 1)) {
        foreach ($part in @($RequestTarget.Substring($question + 1) -split '&')) {
            ...
            if (-not $values.ContainsKey($name)) {
                throw 'BF-906 BLOCKED: Player Compare accepts only left, right, and q query parameters.'
            }
            $values[$name] += [System.Uri]::UnescapeDataString($rawValue.Replace('+', ' ')).Trim()
        }
    }

    foreach ($name in @('left','right','q')) {
...
function ConvertTo-PlayerCompareView {
    ...
    foreach ($required in @($league,$season,$ageAsOf,$valueSource)) {
...
function ConvertTo-PlayerCompareCardHtml {
    ...
    $flagsHtml = ConvertTo-PlayerCompareFlagsHtml -Player $Player
...
<details><summary>Supporting evidence</summary><div class="compare-flags">$flagsHtml</div></details>
</article>
"@
}
...
"@
    ...
    if ($null -ne $View) {
        ...
        $rightHtml = ConvertTo-PlayerCompareCardHtml -Player $View.Right
        return @"
...
<section class="panel"><div class="compare-grid">$leftHtml$rightHtml</div><details><summary>Comparison evidence</summary><div class="technical">season $(ConvertTo-HtmlText $View.Season) &middot; age as-of $(ConvertTo-HtmlText $View.AgeAsOf) &middot; value source $(ConvertTo-HtmlText $View.ValueSource)</div></details></section>
