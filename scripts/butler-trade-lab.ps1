...
function ConvertTo-TradeAssetCheckboxes {
    ...
    $playerAssets = @($Team.Assets | Where-Object { $_.Type -ceq 'player' } | Sort-Object Label)
    $pickAssets = @($Team.Assets | Where-Object { $_.Type -cne 'player' } | Sort-Object Label)

    if (($playerAssets.Count + $pickAssets.Count) -eq 0) {
        return '<div class="empty">No persisted assets are available for this team.</div>'
    }

    $groupsHtml = ''
    foreach ($group in @(
        [pscustomobject]@{ Class = 'players'; Label = 'Players'; Assets = @($playerAssets); Empty = 'No player assets are available.' },
        [pscustomobject]@{ Class = 'picks'; Label = 'Draft picks'; Assets = @($pickAssets); Empty = 'No draft picks are available.' }
    )) {
        $optionsHtml = ''
        foreach ($asset in @($group.Assets)) {
            ...
            $optionsHtml += "<label class=`"asset-option`"><input type=`"checkbox`" name=`"$Name`" value=`"$(ConvertTo-HtmlText $asset.Token)`"$checked><span><strong>$(ConvertTo-HtmlText $asset.Label)</strong><small>$(ConvertTo-HtmlText ($asset.Type.ToUpperInvariant())) &middot; <span class=`"$valueClass`">$(ConvertTo-HtmlText $valueText)</span> &middot; $(ConvertTo-HtmlText $asset.Slot)</small></span></label>"
        }
        if ([string]::IsNullOrWhiteSpace($optionsHtml)) {
            $optionsHtml = "<div class=`"empty asset-group-empty`">$(ConvertTo-HtmlText $group.Empty)</div>"
        }

        $groupOpen = if ($group.Class -ceq 'players') { ' open' } else { '' }
        $groupsHtml += "<details class=`"asset-group $($group.Class)`"$groupOpen><summary class=`"asset-group-head`"><h4>$(ConvertTo-HtmlText $group.Label)</h4><span class=`"asset-count`">$(@($group.Assets).Count)</span></summary><div class=`"asset-list`">$optionsHtml</div></details>"
    }

    return "<div class=`"asset-groups`">$groupsHtml</div>"
}
...
function ConvertTo-TradeLabHtml {
    ...
    $css = Get-AppCss
    $tradeCss = @'
.trade-setup{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:end;margin-top:16px}.field label{display:block;color:var(--muted);font-size:10px;font-weight:900;letter-spacing:.07em;text-transform:uppercase;margin-bottom:6px}.field select,.trade-button{font:inherit;border-radius:var(--radius);border:1px solid var(--line);background:var(--surface);color:var(--ink);padding:10px 12px}.field select{width:100%}.trade-button{cursor:pointer;background:var(--turf);border-color:var(--turf);color:#fff;font-weight:900;text-transform:uppercase;letter-spacing:.05em}.trade-button:hover{background:var(--turf-deep);border-color:var(--turf-deep)}.trade-columns{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;margin-top:16px}.asset-groups{display:grid;gap:14px;margin-top:12px}.asset-group{padding:12px;border:1px solid var(--line);border-radius:10px;background:var(--surface-2)}.asset-group-head{display:flex;align-items:center;justify-content:space-between;gap:10px}.asset-group>summary{cursor:pointer;list-style:none}.asset-group>summary::-webkit-details-marker{display:none}.asset-group-head h4{margin:0;font-size:13px}.asset-count{display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:24px;padding:0 8px;border:1px solid var(--line);border-radius:999px;background:var(--surface);color:var(--muted);font-size:11px;font-weight:900}.asset-list{display:grid;gap:8px;margin-top:8px}.asset-group-empty{margin-top:2px}.asset-option{display:flex;gap:10px;align-items:flex-start;padding:12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--surface);cursor:pointer}.asset-option:hover{background:var(--surface-2)}.asset-option input{margin-top:4px}.asset-option strong{display:block}.asset-option small{display:block;color:var(--muted);margin-top:3px}.asset-value{color:var(--turf)}.asset-missing{color:#8a6319}.trade-submit{margin-top:16px}.trade-result{border-left:4px solid var(--turf)}.trade-result>.empty{font-size:14px;color:var(--ink)}.trade-proof{margin-top:14px}.trade-proof>summary{font-size:13px}.trade-proof-body{margin-top:12px}.counter-form{margin-top:14px;display:flex;flex-wrap:wrap;align-items:center;gap:10px}.counter-form .meta{margin:0}.counter-result{border-left:4px solid var(--gold)}.counter-message{margin-top:14px;padding:14px 16px;border:1px solid var(--line);border-radius:10px;background:var(--surface-2);font-size:16px;line-height:1.55}.counter-packages{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px}.counter-package{padding:13px;border:1px solid var(--line);border-radius:10px;background:var(--surface-2)}.counter-package strong{display:block;color:var(--muted);font-size:10px;text-transform:uppercase;letter-spacing:.05em}.counter-package span{display:block;margin-top:5px;font-weight:800}.gate-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:0;margin-top:14px;border-top:1px solid var(--line);border-left:1px solid var(--line)}.gate{padding:10px;border-right:1px solid var(--line);border-bottom:1px solid var(--line);background:var(--surface);font-size:12px}.gate strong{display:block;color:var(--muted);font-size:9px;text-transform:uppercase;letter-spacing:.06em}.gate span{display:block;margin-top:4px;font-weight:900}.raw-output{white-space:pre-wrap;word-break:break-word;background:var(--surface-2);border:1px solid var(--line);border-radius:var(--radius);padding:12px;color:var(--ink);font:12px Consolas,monospace}.veto-list{display:grid;gap:8px;margin-top:10px}.veto-item{padding:10px;border:1px solid var(--brick);border-radius:var(--radius);background:color-mix(in srgb,var(--brick) 7%,var(--surface));color:var(--brick)}.deal-snapshot-panel{border-left:4px solid var(--turf)}.deal-snapshot{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px}.deal-side{padding:14px 15px;border:1px solid var(--line);border-radius:10px;background:var(--surface-2)}.deal-side strong{display:block;color:var(--muted);font-size:10px;text-transform:uppercase;letter-spacing:.05em}.deal-side span{display:block;margin-top:6px;font-weight:800;line-height:1.4}.decision-reason{margin-top:14px;padding:13px 15px;border:1px solid var(--line);border-radius:10px;background:var(--surface-2)}.decision-reason strong{display:block;color:var(--muted);font-size:10px;text-transform:uppercase;letter-spacing:.05em}.decision-reason span{display:block;margin-top:5px;font-weight:700}.edit-deal{margin:0 0 18px;border:1px solid var(--line);border-radius:12px;background:var(--surface)}.edit-deal>summary{padding:14px 16px;cursor:pointer;color:var(--turf-deep);font-weight:800}.edit-deal>.panel{margin:0;border:0;border-top:1px solid var(--line);border-radius:0;box-shadow:none}@media(max-width:800px){.trade-setup,.trade-columns,.counter-packages,.deal-snapshot{grid-template-columns:1fr}.gate-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
'@
...
"@
    ...
    $dealSnapshotHtml = ''
    $builderPresentation = $builder
    $resultHtml = ''
    $counterHtml = ''
    if ($null -ne $Evaluation) {
        if ($null -eq $Opponent) {
            throw 'BF-910 BLOCKED: evaluated Trade Hub rendering requires the exact selected opponent.'
        }
...
"@
        ...
        $reasonHtml = if ([string]::IsNullOrWhiteSpace($Evaluation.Reason)) { '' } else { "<div class=`"decision-reason`"><strong>Why</strong><span>$(ConvertTo-HtmlText $Evaluation.Reason)</span></div>" }
