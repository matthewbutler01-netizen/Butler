...
function ConvertTo-TradeAssetCheckboxes {
    ...
    foreach ($group in @(
        ...
        $groupsHtml += "<section class=`"asset-group $($group.Class)`"><div class=`"asset-group-head`"><h4>$(ConvertTo-HtmlText $group.Label)</h4><span class=`"asset-count`">$(@($group.Assets).Count)</span></div><div class=`"asset-list`">$optionsHtml</div></section>"
    }
