...
<section class="panel">
  ...
  <div class="board-disclaimer">Status, injury, depth, and market attention are descriptive only. Market attention is descriptive only and is not Butler's score. Newcomers remain nonnumeric. If shown, <strong>Current governed ADD</strong> identifies the already-audited current add and <strong>Paired audited DROP</strong> identifies only its exact audited counterpart; the paired context is traceability only, and the current ADD itself does not alter BF-616 order or rank the board.</div>
