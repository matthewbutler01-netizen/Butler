...
@media(max-width:920px){.evidence-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.priority-card{grid-template-columns:42px minmax(0,1fr)}.priority-card>.status{grid-column:2;justify-self:start}}
@media(max-width:760px){.command-head,.section-head,.decision-strip{display:block}.command-title{font-size:27px}.quick-grid,.evidence-grid,.priority-move{grid-template-columns:1fr}.priority-card{grid-template-columns:36px minmax(0,1fr);padding:14px}.priority-card>.status{grid-column:2}.command-summary{gap:6px}.manager-actions{margin-top:12px}}
'@
...
if ($newReturn -match 'Sleeper ID|Audit ID|BF-\d|gradlew|textarea|UUID') {
    throw 'BF-804 BLOCKED: manager Command Center markup contains operator or identity noise.'
}
