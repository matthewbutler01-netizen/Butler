...
try {
    ...
    # The core pool preselects the next backend before AcceptTcpClient. Sequential
    # requests therefore alternate across two preserved cores. Prime both first.
    $prime1Ms = Invoke-Bf750Get -Port $port -RequestTarget ("/league?bf750=prime1-{0}" -f $nonce)
    $prime2Ms = Invoke-Bf750Get -Port $port -RequestTarget ("/league?bf750=prime2-{0}" -f $nonce)
    ...
    Write-Host ''
    Write-Host ("BF-750 warm core-pool route timing: prime1_ms={0}; prime2_ms={1}; team_ms={2}; waivers_ms={3}; league_ms={4}; samples={5}" -f `
        $prime1Ms, $prime2Ms, $teamMedian, $waiverMedian, $leagueMedian, $Samples)
    Write-Host 'BF-750 diagnostic boundary: two naturally alternating preserved cores are primed, then sequential GET-only production-path reads use diagnostic query strings; outer exact-request single-flight cache bypassed only for measurement; /refresh excluded; no Butler or Sleeper write path is invoked.'
}
finally {
    if ($null -ne $process) {
        try {
